				...
				<el-form ref="infoForm" :rules="infoRules" :model="infoForm" label-width="120px">
					...
					<el-form-item label="商品轮播图" prop="goods_sn">
						<draggable v-model="gallery_list" draggable=".gallery-item" class="drag-wrap">
							...
							<el-upload name="file" ref="upload" :on-remove="galleryRemove" class="upload-demo" :action="uploadFileUrl"
					...
					<el-form-item class="upload_ad">
						<el-upload ref="upload" name="file" class="avatar-uploader" :action="uploadFileUrl" list-type="picture-card"
...
export default {
  ...
  methods: {
      ...
      lrz(file)
        .then((rst) => {
          ...
          that.generateFilename(file, (filename) => {
            ...
            this.$http
              .post(this.uploadFileUrl, formData, config)
              .then((res) => {
    ...
    handleUploadListSuccess(res) {
      let url = this.url;
      this.infoForm.list_pic_url = url + res.data.name;
      this.infoForm.https_pic_url = res.data.url;
    ...
    uploadGalleryImg(request) {
      ...
      lrz(file)
        .then((rst) => {
          ...
          //   const fileName = file.name; //moment().format('YYYYMMDDHHmmssSSS') + Math.floor(Math.random() * 100) + file.name; //自定义图片名
          that.generateFilename(file, (filename) => {
            const formData = new FormData();
            formData.append("file", rst.file);
            // formData.append("token", this.picData.token);
